...
int ConvertHeight(unsigned char brightness)
{
	// Apply exponential scale to height data.
	return (int)((float)brightness*TILE_Y_SCALE);
	//return brightness;
}
...
void LoadJPGMap(const char* relative)
{
	...
	g_hmap.alloc((pImage->sizeX-1), (pImage->sizeY-1));
	...
	for(int x=0; x<pImage->sizeX; x++)
	{
		for(int z=0; z<pImage->sizeY; z++)
		{
			...
			// Apply exponential scale to height data.
			unsigned char y = ConvertHeight((unsigned char)((r+g+b)/3.0f));
