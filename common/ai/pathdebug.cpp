...
void DrawGrid()
{
	...
	if(py->sel.units.size() > 0)
	{
		...
		for(int x=imax(0, ux-50); x<imin(ux+50, g_pathdim.x); x++)
			for(int z=imax(0, uz-50); z<imin(uz+50, g_pathdim.y); z++)
			{
				...
				vecs.push_back(Vec3f(x*PATHNODE_SIZE, 1.0f, z*PATHNODE_SIZE));
				vecs.push_back(Vec3f((x+1)*PATHNODE_SIZE, 1.0f, (z+1)*PATHNODE_SIZE));
				vecs.push_back(Vec3f((x+1)*PATHNODE_SIZE, 1.0f, z*PATHNODE_SIZE));
				vecs.push_back(Vec3f(x*PATHNODE_SIZE, 1.0f, (z+1)*PATHNODE_SIZE));
	...
	for(int z=0; z<g_pathdim.y-1; z++)
	{
		...
		{
			int i = gridvecs.size();
			gridvecs.push_back(Vec3f((float)x*PATHNODE_SIZE, 0.0f + 1, (float)z*PATHNODE_SIZE));
			gridvecs.push_back(Vec3f((float)(x+1)*PATHNODE_SIZE, 0.0f + 1, (float)z*PATHNODE_SIZE));
			gridvecs[i+0].y = g_hmap.accheight(gridvecs[i+0].x, gridvecs[i+0].z) + TILE_SIZE/10;
			gridvecs[i+1].y = g_hmap.accheight(gridvecs[i+1].x, gridvecs[i+1].z) + TILE_SIZE/10;
