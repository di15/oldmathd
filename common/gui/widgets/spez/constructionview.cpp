...
void ConstructionView::regen(Selection* sel)
{
	...
	if(sel->buildings.size() > 0)
	{
		...
		//RichText newtitle;
		//newtitle.m_part.push_back( RichPart(t->name) );
		//newtitle.m_part.push_back( RichPart(" Construction Manager") );

		//titletext.m_text = RichText(t->name);
		//titletext.m_text.m_part.push_back( RichPart(" Construction Manager") );
	...
	else if(sel->roads.size() > 0)
	{
		...
		RoadTile* road = GetCo(CONDUIT_ROAD, xz.x, xz.y);
	...
	if(conmat)
	{
		...
		for(; i<RESOURCES; i++)
		{
			...
			textline1rt.m_part.push_back( RichPart(textline1cstr) );
			textline1rt.m_part.push_back( RichPart(RICHTEXT_ICON, g_resource[i].icon) );
			textline1rt.m_part.push_back( RichPart(g_resource[i].name.c_str()) );
			...
			textline2rt.m_part.push_back( RichPart("Max. est. cost:") );
			reqrow.textline2 = Text(this, "textline2", textline2rt, font, NULL, false, color[0], color[1], color[2], color[3]);

			RichText dollarsignrt;
			dollarsignrt.m_part.push_back( RichPart(RICHTEXT_ICON, g_resource[RES_FUNDS].icon) );
			...
			RichText editrt;
			editrt.m_part.push_back( RichPart(edittext) );
			...
			perunitrt.m_part.push_back( RichPart(perunitcstr) );
		...
		while(true)
		{
			...
			RichText textline2rt;
			textline2rt.m_part.push_back( RichPart("Construction wage:") );
			reqrow.textline2 = Text(this, "textline2", textline2rt, font, NULL, false, color[0], color[1], color[2], color[3]);
			...
			perunitrt.m_part.push_back( RichPart("/") );
			perunitrt.m_part.push_back( RichPart(RICHTEXT_ICON, g_resource[i].icon) );
			perunitrt.m_part.push_back( RichPart(perunitcstr2) );
			reqrow.perunittext = Text(this, "perunittext", perunitrt, font, NULL, false, color[0], color[1], color[2], color[3]);
...
void ConstructionView::reframe()	//resized or moved
{
	...
	bg_touch.m_pos[0] = 0;
	bg_touch.m_pos[1] = 0;
	bg_touch.m_pos[2] = g_width;
	bg_touch.m_pos[3] = g_height;
	...
	int rowi = 0;
	int gheight = (int)g_font[MAINFONT16].gheight;
	for(auto i=reqrows.begin(); i!=reqrows.end(); i++)
	{
		int line1y = rowi * gheight*3 + (int)(m_pos[1]+m_pos[3])/2 + 10;
		...
		float endx = (float)EndX(&dollarsign->m_text, dollarsign->m_text.texlen(), dollarsign->m_font, dollarsign->m_pos[0], dollarsign->m_pos[1]);
		...
		perunittext->m_pos[1] = (float)line2y;
		perunittext->m_pos[2] = m_pos[2] - 10;
		perunittext->m_pos[3] = (float)line2y + (float)gheight*2;
