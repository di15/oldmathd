...
void Resize_ResTicker(Widget* thisw)
{
	...
	thisw->m_pos[2] = -90000;
	thisw->m_pos[3] = -90000;
	thisw->m_tpos[0] = -90000;
	thisw->m_tpos[1] = -90000;
#endif
}
...
void BuildMenu_OpenPage1()
{
	...
	bp->bottomright_button[6] = Button(bp, "name", "gui/brbut/mine.png", RichText(""), RichText(""), MAINFONT8, BUTTON_CORRODE, NULL, NULL, Click_BuildButton, NULL, Over_BuildButton, Out_BuildButton, BL_SHMINE);
...
#elif 1
	thisw->m_pos[0] = g_width - 400;
	thisw->m_pos[1] = g_height - MINIMAP_SIZE - 32 - 400;
	thisw->m_pos[2] = g_width;
...
void Click_MoveConstruction()
{
	...
	{
		Vec2i tpos = *siter;
		RoadTile* r = GetCo(CONDUIT_ROAD, tpos.x, tpos.y);
...
void Resize_Message(Widget* thisw)
{
	...
	thisw->m_pos[1] = g_height/2 - 100;
	thisw->m_pos[2] = g_width/2 + 200;
	thisw->m_pos[3] = g_height/2 + 100;
}

void Resize_MessageContinue(Widget* thisw)
{
	Player* py = &g_player[g_curP];
	thisw->m_pos[0] = g_width/2 - 80;
	thisw->m_pos[1] = g_height/2 + 70;
	thisw->m_pos[2] = g_width/2 + 80;
	thisw->m_pos[3] = g_height/2 + 90;
}
...
void FillPlayGUI()
{
	...
	playview->add(new ResTicker(NULL, "res ticker", Resize_ResTicker));
	playview->add(new BottomPanel(NULL, "bottom panel", Resize_BottomPanel));
